...
double binomial(int nPSusers, int num)
{
    return factorial(nPSusers) / (factorial(num) * factorial(nPSusers - num));
}
...
int main(int argc, char *argv[])
{
    ...
    /*
        ...
        * Description: This shows the difference between circuit and packet switching
        */
    ...
    double pAnyTenBusy = binomial(nPSusers, 10) * pow(pPSusers, 10) * pow(pPSusersNotBusy, nPSusers - 10);
    double pTenMoreBusy = 0;

    for (int i = 11; i <= nPSusers; i++)
    {
        pTenMoreBusy += binomial(nPSusers, i) * pow(pPSusers, i) * pow(pPSusersNotBusy, nPSusers - i);
    }
